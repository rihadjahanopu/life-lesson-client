...
import { z } from 'zod';
import { ImagePlus, Crown, Smile, ArrowLeft } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        ...
        setDescText(lesson.description || '');
        if (lesson.image) setImagePreview(lesson.image);
      } catch {
  ...
  return (
    <div>
      <button type="button" onClick={() => router.back()}
        className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-violet-600 mb-4">
        <ArrowLeft size={16} /> Back to My Lessons
      </button>

      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Edit Lesson</h1>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">Update your lesson content</p>
