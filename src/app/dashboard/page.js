			...
			<div className="flex items-center justify-between mb-8">
				...
				<div className="flex items-center gap-3">
					{!user?.isPremium && (
						<Link
							href="/pricing"
							className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-medium rounded-xl hover:from-yellow-600 hover:to-orange-600 text-sm">
							<Crown size={16} /> Upgrade
						</Link>
					)}
					<Link
						href="/dashboard/add-lesson"
						className="flex items-center gap-2 px-4 py-2 bg-violet-600 text-white font-medium rounded-xl hover:bg-violet-700 text-sm">
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
				...
				<div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 flex flex-col items-center justify-center text-center">
					...
					<p className="text-xs text-gray-400 mt-2">
						Keep creating lessons daily to maintain your streak!
					</p>
				</div>
						...
						<div className="flex gap-1 min-w-fit">
							...
							<div className="flex flex-col gap-1 mr-1">
								{dayLabels.map((day, i) => (
									<div
										key={day}
										className="w-8 h-3.5 flex items-center justify-end pr-1">
										{i % 2 === 1 && (
							...
							<Link
								...
								<div>
									...
									<p className="text-xs text-gray-500">
										{l.category} • {formatDateShort(l.createdAt)}
									</p>
								</div>
								<div className="flex items-center gap-4 text-xs text-gray-400">
									<span className="flex items-center gap-1">
										<Eye size={12} /> {l.viewsCount}
									</span>
									<span className="flex items-center gap-1">
										<Heart size={12} /> {l.likesCount}
									</span>
								</div>
							</Link>
						))}
