...
import { Crown, BookOpen, Heart, Mail, Shield, Camera, Edit2, X, Loader2, Save, Monitor, Smartphone, Trash2, Laptop, Activity } from 'lucide-react';
import { lessonService, favoriteService, userService } from '@/services';
...
export default function ProfilePage() {
  ...
  const handleRevokeSession = async (sessionId) => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.response?.data?.error || 'Failed to revoke session', { id: toastId });
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-violet-600 to-purple-700 p-8 relative">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="relative group">
              {photoURL ? (
                <img src={photoURL} alt={user?.name} className="w-24 h-24 rounded-full border-4 border-white/20 object-cover" />
              ) : (
          ...
          <form onSubmit={handleSave} className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">You can upload a file directly or paste an external image link.</p>
              </div>
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4 p-6 bg-gray-50/50 dark:bg-gray-800/50">
              <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200/50 dark:border-gray-700 shadow-sm">
                <BookOpen size={24} className="text-violet-500 mx-auto mb-2" />
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{myLessons.length}</p>
              ...
              <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200/50 dark:border-gray-700 shadow-sm">
                <Mail size={24} className="text-blue-500 mx-auto mb-2" />
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{publicLessons.length}</p>
            ...
            <div className="p-6 border-t border-gray-200 dark:border-gray-700 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white font-medium">{user?.isPremium ? 'Active' : 'Not Premium'}</p>
                </div>
              </div>
            </div>
          </>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm p-6">
        <div className="flex items-center gap-2 mb-6 pb-4 border-b border-gray-150 dark:border-gray-750">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Active Sessions &amp; Devices</h3>
            <p className="text-xs text-gray-450 dark:text-gray-400 mt-0.5">Manage your active account sessions and revoke access for other devices.</p>
          </div>
        </div>
        ...
        {sessionsLoading ? (
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700/60">
            {sessions.map((s) => {
              const isMobile = s.userAgent?.toLowerCase().includes('mobi') || s.userAgent?.toLowerCase().includes('android') || s.userAgent?.toLowerCase().includes('iphone');
              return (
                <div key={s.id} className="py-4 flex items-center justify-between gap-4 first:pt-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-gray-900 dark:text-white">{s.deviceInfo}</p>
                        {s.isCurrent && (
                      ...
                      <p className="text-xs text-gray-450 mt-1">IP Address: <span className="font-mono text-gray-500 dark:text-gray-400">{s.ipAddress}</span></p>
                    </div>
                  </div>
          ...
          <p className="text-center py-6 text-sm text-gray-500 dark:text-gray-400">No active sessions found.</p>
        )}
      </div>
    </div>
  );
